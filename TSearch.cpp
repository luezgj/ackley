...
int clipped = 0; // cantidad de veces que se vuelve a los bordes
...
TSearch::TSearch(int VSize, double (*EvalFn)(TVector<double>&, RandomState&))
{
	...
	parentTournSize = PARENTS_TOURNAMENT_SIZE;
	crossPointsAmount = NCrossPoints;
	SearchInitialized = 0;
	...
	SetParentSelectionMode(SUS);
	SetReproductionMode(GENETIC_ALGORITHM_CANDADAI);
	...
	SetMutationInputBiasVariance(1.0);
...
void TSearch::SetPopulationSize(int NewSize)
{
	...
	//if ((NewSize % 2) != 0) { cerr << "Invalid population size, it must be an even number: " << NewSize; exit(0); }
...
void TSearch::SetMutationInputBiasVariance(double NewVariance)
{
	if (NewVariance <= 0.0) {
		cerr << "Invalid Input Bias MutationVariance: " << NewVariance;
		exit(0);
	}
	InputBiasMutationVar = NewVariance;
}
...
void TSearch::DoSearch(int ResumeFlag)
{
	...
	if (!ResumeFlag) {
		...
		EvaluatePopulation(Population,Perf);
		BestPerf = (maximize?-1:100);
		UpdateBestFlag = 0;
	}
	...
	// Display results
	cout <<"Cantidad de veces que se metió a los rangos" <<clipped;
...
{
	for (int i = 1; i <= evolvableNeurons*M ; i++)
		v[i] = rs.UniformRandom(MinSearchValue, MaxSearchValue);
	//rateCodeGain
	v[genotypeSize - 2] = rs.UniformRandom(MinInputBias, MaxInputBias);
	//inputBias
	v[genotypeSize - 1] = rs.UniformRandom(MinRateCode, MaxRateCode);
	//WindowsSize
	v[genotypeSize] = rs.UniformRandomInteger(MinWindowLength, MaxWindowLength);
}
...
void TSearch::UpdatePopulationStatistics(void)
{
	...
	for (i = 1; i <= Population.Size(); i++)
	{
		...
		if (perf > MaxPerf) { MaxPerf = perf; if (maximize) { bestindex = i; } }
		if (perf < MinPerf) { MinPerf = perf; if (!maximize) { bestindex = i; } }
		// Update total
		total += perf;
	}
	...
	if (maximize && (ReEvalFlag || (MaxPerf > BestPerf))) 
	{
		UpdateBestFlag = 1;
		BestPerf = MaxPerf;
		bestVector = Population[bestindex];
	}else if ((!maximize) && (ReEvalFlag ||  (MinPerf < BestPerf)))
	{
		UpdateBestFlag = 1;
		BestPerf = MinPerf;
		bestVector = Population[bestindex];
...
void* EvaluatePopulationRange(void* arg)
{
	...
	TVector<TVector<double>>* pop = prs->population;
	TVector<double>* perf = prs->performances;
	for (int i = prs->start; i <= prs->end; i++)
		perf->operator[](i)= s->EvaluateVector(pop->operator[](i), s->RandomStates[i]);
	pthread_exit(NULL);
	return NULL;
}

void printPop(TVector<TVector<double>>& population) {
	cout << "Evaluando la problación:"<<endl;
	for (int i = 1; i <= population.Size();i++) {
		cout << "Ind" << i << ": ";
		for (int gen = 1; gen <= genotypeSize; gen++) {
			cout << population[i][gen] << "\ ";
		}
		cout << endl;
	}
	cout << endl;
...
void TSearch::printPerf(TVector<double>& performances) {
...
// Evaluate the parameter population, beginning with the STARTth individual

void TSearch::EvaluatePopulation(TVector<TVector<double>> & population, TVector<double> & performances,int start)
{
	...
	if (THREAD_COUNT > 1) {
		...
		for (int i = 1; i <= THREAD_COUNT - 1; i++) {
			...
			psrs[i - 1].performances = &performances;
			psrs[i - 1].population = &population;
		...
		for (int i = (THREAD_COUNT - 1) * NumIndividuals + start; i <= PopulationSize(); i++)
			performances[i] = EvaluateVector(population[i], RandomStates[i]);
...
void TSearch::UpdatePopulationFitness(TVector<TVector<double>> & population, TVector<double>& perf, TVector<double>& fitn, bool sorted)
{
	int psize = population.Size();
	if (!sorted)
		SortPopulation(population, perf);
	switch (SelectMode) {
	...
	case FITNESS_PROPORTIONATE:
	{
		...
		{
			fitn[i] = m * (perf[i] - AvgPerf) + AvgPerf;
			total = total + fitn[i];
		}
		for (int i = 1; i <= psize; i++)
			fitn[i] = fitn[i] / total;
	...
	case RANK_BASED:{
		cout << "Fitness"<< endl;
		for (int i = 1; i <= psize; i++){
			fitn[i] = (MaxExpOffspring + (2.0 - 2.0 * MaxExpOffspring) * ((i - 1.0) / (psize - 1))) / psize;
			cout << fitn[i] << " ";
		}
		break;
	}
	case RANK_BASED_GIMENEZ:{
		//cout << "Fitness" << endl;
		int pos;
		double acum;
		acum = 0.0;
		for (int i = 1; i <= psize; i++) {
			/*Esto no anda bien. Ver de copiar alguno que ya esté hecho. Creo que no anda xq hay que guardar en fitn el fitness normal, no el acumulado (creo que esa cuanta lo va acumulando)
			fitn[i] =(2.0-SelectivePressure)+2.0*(SelectivePressure-1.0)*((pos-1)/(psize -1));
			*/
			fitn[i] = 1.0 / sqrt(double(i + 1));
			acum += fitn[i];
		}
		for (int i = 1; i <= psize; i++) {
			fitn[i] = fitn[i] / acum;
			pos = psize - i + 1;
			//cout << "i:" << i << " pos:" <<pos << "perf:" <<perf[i] <<" fitn:" << fitn[i] << endl;
		}
		break;
	}
...
{
	double magnitude;
	TVector<double> TempVector(1, evolvableNeurons * M);
	...
	//mutate input bias
	magnitude = rs.GaussianRandom(0.0, InputBiasMutationVar);
	if (ConstraintVector[genotypeSize - 2])
		v[genotypeSize - 2] = clip(v[genotypeSize - 2] + magnitude, MinInputBias, MaxInputBias);
	else
		v[genotypeSize - 2] = v[genotypeSize - 2] + magnitude;
	//mutate rate code gain
	magnitude = rs.GaussianRandom(0.0, mutationVarianceRateCodeGain);
	if (ConstraintVector[genotypeSize - 1])
		v[genotypeSize - 1] = clip(v[genotypeSize - 1] + magnitude, MinRateCode, MaxRateCode);
	else
		v[genotypeSize - 1] = v[genotypeSize - 1] + magnitude;
	//mutate window size
	if(rs.ProbabilisticChoice(MutationVar)){
		magnitude = rs.UniformRandomInteger(-2, 2);
		if (ConstraintVector[genotypeSize])
			v[genotypeSize] = clip(v[genotypeSize] + magnitude, MinWindowLength, MaxWindowLength);
		else
			v[genotypeSize] = v[genotypeSize] + magnitude;
	}
}


void TSearch::MutateVectorGimenez(TVector<double>& v)
{
	double magnitude;

	// Apply the mutation to V weigths
	for (int i = 1; i <= evolvableNeurons*M; i++){
		// Generate a normally-distributed random magnitude
		magnitude = rs.GaussianRandom(0.0, MutationVar);
		if (ConstraintVector[i])
...
// Perform a modular n-point crossover between two individuals

void TSearch::NPointCrossover(TVector<double>& v1, TVector<double>& v2)
{
	int crossPoints[MaxCrossPoints];
	for (int i = 0; i < crossPointsAmount; i++) {
		int point = rs.UniformRandomInteger(1, vectorSize);
		while (find(begin(crossPoints), end(crossPoints), point) != end(crossPoints)) {
			point = rs.UniformRandomInteger(1, vectorSize);
		}
		crossPoints[i] = point;
	}
	//fill array
	for (int i = crossPointsAmount; i < MaxCrossPoints; i++) {
		crossPoints[i] = INT_MAX;
	}

	sort(begin(crossPoints), end(crossPoints));

	int currentIndex = 1;
	for (int i = 0; i < crossPointsAmount; i++) {
		if ((i % 2) == 0) {
			for (currentIndex; currentIndex < crossPoints[i];currentIndex++) {
				double temp = v1[currentIndex];
				v1[currentIndex] = v2[currentIndex];
				v2[currentIndex] = temp;
			}
		}
		else {
			currentIndex = crossPoints[i];
		}
	}
}
...
void TSearch::ReproducePopulation(void)
{
	...
	case GENETIC_ALGORITHM_CANDADAI: ReproducePopulationGeneticAlgorithm(); break;
	case GENETIC_ALGORITHM_GIMENEZ: ReproducePopulationGeneticAlgorithmGimenez(); break;
...
void TSearch::ReproducePopulationHillClimbing(void)
{
	...
	UpdatePopulationFitness(Population,Perf,fitness);
...
void TSearch::ReproducePopulationGeneticAlgorithm(void)
{
	...
	while (i <= psize) {
		...
		if (ProbabilisticChoice(CrossProb) && (i < psize)) {
			...
			case N_POINT: NPointCrossover(Population[i], Parent2); break;
	...
	if (ReEvalFlag) EvaluatePopulation(Population,Perf);
	else EvaluatePopulation(Population, Perf,ElitePop + 1);
}


void TSearch::ReproducePopulationGeneticAlgorithmGimenez(void)
{
	int psize = PopulationSize();

	// Calculate population fitness
	UpdatePopulationFitness(Population, Perf, fitness, false);
	//printPerf(Perf);
	// Determine the number of elite individuals in the new population
	int ElitePop = ((int)floor(EFraction * psize + 0.5));
	// Select the rest of the population using Baker's stochastic universal sampling
	TVector<TVector<double> > Offspring = Population;
	...
	case TOURNAMENT:{
		...
		for (int i = 1 ; i <= psize; i++) {
			int individual;
			double mixPerformance = 0.0;   //mix= max/min
			TVector<double> mixIndividual;
			...
			for (int competitor = 1; competitor <= PARENTS_TOURNAMENT_SIZE; competitor++) {
				mixPerformance = (maximize?0.0:40.0);
				...
				if (  (maximize && (Perf[individual] > mixPerformance))
				   || (!maximize && (Perf[individual] < mixPerformance))) {
						mixPerformance = Perf[individual];
						mixIndividual = Population[individual];
				}
			}
			Offspring[i] = mixIndividual;
	...
	while (i <= psize) {
		...
		if (ProbabilisticChoice(CrossProb) && (i < psize)) {
			...
			switch (CrossMode) {
			case UNIFORM: UniformCrossover(Offspring[i], Offspring[i+1]); break;
			case TWO_POINT: TwoPointCrossover(Offspring[i], Offspring[i+1]); break;
			case N_POINT: NPointCrossover(Offspring[i], Offspring[i + 1]); break;
			default: cerr << "Invalid crossover mode" << endl; exit(0);
			}
			// If the child is the same as parent after crossover, mutate it
			if (EqualVector(Offspring[i], Parent1)) MutateVectorGimenez(Offspring[i]);
			if (EqualVector(Offspring[i+1], Parent2)) MutateVectorGimenez(Offspring[i+1]);
		}
		// Perform mutation
		MutateVectorGimenez(Offspring[i]);
		MutateVectorGimenez(Offspring[i+1]);
		i = i + 2;
	}

	//Select and evaluate next population
	SelectNextPopulation(ElitePop,Population, Offspring);
}
...
inline int partition(int first, int last, TVector<double>& perf, TVector<TVector<double> >& pop)
{
	...
	for (int i = first; i <= last; i++) {
			...
			/*Con > maximiza la performance*/
			if ((maximize && (perf[i] >  pivot_value)) 
			|| (!maximize && (perf[i] < pivot_value))) {
				pivot++;
				if (i != pivot) {
...
void TSearch::SortPopulation(TVector<TVector<double>>& population, TVector<double>& perf)
{
	quicksort(1, population.Size(), perf, population);
}

//At end set next population into parents vector.
void TSearch::SelectNextPopulation(int eliteCount, TVector<TVector<double> >& parents, TVector<TVector<double> >& offsprings) {
	TVector<TVector<double> > TempPopulation;
	TempPopulation.SetBounds(1,parents.Size());

	TVector<double> TempPerf;
	TempPerf.SetBounds(1, parents.Size());

	TVector<double > OffspringPerf;
	OffspringPerf.SetBounds(1, offsprings.Size());

	EvaluatePopulation(offsprings, OffspringPerf);

	SortPopulation(offsprings,OffspringPerf);

	//elite passes to next population
	for (int i = 1; i <= eliteCount;i++) {
		TempPopulation[i] = parents[i];
		TempPerf[i] = Perf[i];
		TempPopulation[i + eliteCount] = offsprings[i];
		TempPerf[i + eliteCount] = OffspringPerf[i];
	}

	//complete population with SUS (no está terminado)
	//Hay que calcular el fitness (que suma 1) con los individuos que restan de padres e hijos, y después elegir
	/*TVector<TVector<double>> rest;
	TVector<double> perf;
	TVector<double> fit;

	COPIAR ORDENADO A REST EL RESTO DE PADRES Y DE HIJOS, TAMBIEN COPIAR LAS PERFORMANCES

	UpdatePopulationFitness(rest, perf, fit, true);

	int j = 1 + eliteCount * 2;
	double sum = 0;
	double rand = rs.UniformRandom(0.0, 1.0);
	for (int i =1; (i <= parents.Size()) && (j <= parents.Size()); i++) {
		sum += (parents.Size()-eliteCount*2) * fitness[i];
		while (rand < sum) {
			TempPopulation[j] = rest[i];
			Perf[j] = perf[i];
			j++;
			rand++;
		}
	...
	//complete population tournament way - MAXIMIZAR
	
	//cout << "Eligiendo sobrevivientes" << endl;

	/*Se elegía algunos de los padres y otros de los hijos. (casi siempre ganaban los padres entonces mejor agarro muchos de los hijos, y algunos de los padres)
	for (int i = 1 + eliteCount * 2; i <= parents.Size() ; i++) {
		int individual;
		bool fromParents = false, MaxFromParents;
		bool used = true;
		double performance;
		TVector<double> gen;
		double maxPerformance=0.0;
		int MaxindividualIndex;
		TVector<double> maxIndividual;
		for (int competitor = 1; competitor <= TOURNAMENT_SIZE; competitor++) {
			maxPerformance=0.0;
			used = true;
			while (used) {
				individual = rs.UniformRandomInteger(eliteCount + 1, parents.Size());
				fromParents = (rs.ProbabilisticChoice(0.5) ? true : false); // select from parents or from offspring
				if (fromParents) {
					if (usedParent[individual]) { used = true; }
					else { used = false; }
				}
				else {
					if (usedOffspring[individual]) { used = true; }
					else { used = false; }
				}
			}

			if (fromParents) {// select from parents or from offspring
				performance = Perf[individual];
				gen = parents[individual];
				//cout << "competidor de padres: " << Perf[individual]<< endl;
			}
			else {
				performance = OffspringPerf[individual];
				gen = offsprings[individual];
				//cout << "competidor de hijos: " << OffspringPerf[individual]<< endl;
			}

			if (performance > maxPerformance) {
				maxPerformance = performance;
				MaxindividualIndex = individual;
				MaxFromParents = fromParents;
				maxIndividual = gen;
			}
		}
		...
		if (MaxFromParents){
			usedParent[MaxindividualIndex] = true;
		}
		else {
			usedOffspring[MaxindividualIndex] = true;
		}
	}
	*/

	int individual, MixindividualIndex;  //Mix= max-min
	double performance;
	TVector<double> gen;
	double mixPerformance;
	TVector<double> mixIndividual;
	...
	int lastOffspringSurvivorIndex = (1 + eliteCount * 2) + fromOffspringCount;
		...
		//fill with offspings
		for (int i = 1 + eliteCount * 2; i <= lastOffspringSurvivorIndex; i++) {
			mixPerformance = (maximize?0.0:40.0);
			for (int competitor = 1; competitor <= TOURNAMENT_SIZE; competitor++) {
				individual = rs.UniformRandomInteger(eliteCount + 1, offsprings.Size());
				...
				if ((maximize && (performance > mixPerformance))
					|| (!maximize && (performance < mixPerformance))) {
					mixPerformance = performance;
					MixindividualIndex = individual;
					mixIndividual = gen;
				}
			}
			TempPopulation[i] = mixIndividual;
			//cout << "Elegido of: " << maxPerformance<< endl;
			TempPerf[i] = mixPerformance;
			used[MixindividualIndex] = true;
		}
		used.FillContents(false);
		//fill with parents
...
void TSearch::WriteCheckpointFile(void)
{
	...
	case RANK_BASED_GIMENEZ: i = 3; break;
	default: cerr << "Invalid selection mode" << endl; exit(0);
	}
	bofs.write((const char*) & (i), sizeof(i));
	// Write the parent selection mode
	switch (ParentSelectMode) {
	case SUS: i = 1; break;
	case TOURNAMENT: i = 2; break;
	default: cerr << "Invalid parent selection mode" << endl; exit(0);
	}
	bofs.write((const char*) & (i), sizeof(i));
	...
	case GENETIC_ALGORITHM_CANDADAI: i = 2; break;
	case GENETIC_ALGORITHM_GIMENEZ: i = 3; break;
	default: cerr << "Invalid reproduction mode" << endl; exit(0);
	}
	bofs.write((const char*) & (i), sizeof(i));
	...
	bofs.write((const char*) & (InputBiasMutationVar), sizeof(InputBiasMutationVar));
...
void TSearch::ReadCheckpointFile(void)
{
	...
	case 3: SetSelectionMode(RANK_BASED_GIMENEZ); break;
	default: cerr << "Invalid selection mode" << endl; exit(0);
	}
	// Read the parent selection mode
	bifs.read((char*) & (i), sizeof(i));
	switch (i) {
	case 1: SetParentSelectionMode(SUS); break;
	case 2: SetParentSelectionMode(TOURNAMENT); break;
	default: cerr << "Invalid parent selection mode" << endl; exit(0);
	}
	...
	case 2: SetReproductionMode(GENETIC_ALGORITHM_CANDADAI); break;
	case 3: SetReproductionMode(GENETIC_ALGORITHM_GIMENEZ); break;
	default: cerr << "Invalid reproduction mode" << endl; exit(0);
	}
	...
	case 3: SetCrossoverMode(N_POINT); break;
	default: cerr << "Invalid crossover mode" << endl; exit(0);
	}
	...
	bifs.read((char*) & (d), sizeof(d));
	SetMutationInputBiasVariance(d);
